...
- A reported transaction price or transaction value of zero does not mean the shares have no economic value.
- When an award, grant, or option transaction has a reported price or value of zero, say: "No transaction price or cash consideration was reported."
- Never say that shares are worthless, have no value, have no monetary value, or have no immediate value.
- Do not estimate the market value of an award or grant unless that value is explicitly included in the input.
...
/*
 * A zero transaction value for an award, grant, or option event
 * does not establish that the securities themselves have no value.
 */
const isCompensationEvent =
  input.tradeType === "award" ||
  input.tradeType === "option";

const reportedValue = Number(
  input.totalValue ??
  input.value ??
  0
);
...
if (
  isCompensationEvent &&
  reportedValue === 0 &&
  unsupportedZeroValueLanguage.test(summary)
) {
  const shares = Number(
    input.totalShares ??
    input.shares ??
  ...
  summary =
    `${input.ceo} reported a ${String(
      input.tradeTypeLabel
    ).toLowerCase()} involving ${shares} shares of ` +
    `${input.company} (${input.ticker}). ` +

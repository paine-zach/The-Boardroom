...
function isRegionalExecutiveTitle(
  value
) {
  const title = normalizeString(
    value,
  ...
  return (
    /(^|[^a-z])regional([^a-z]|$)/.test(
      title
    ) ||
    /southern africa|north america|latin america|asia pacific/.test(
      title
    ) ||
    /(^|[^a-z])(emea|apac)([^a-z]|$)/.test(
      title
    )
  );
}
...
function filingToTransactionRows(
  ...
  /*
   * HermAI's broad CEO match can include a regional CEO. The public
   * product tracks the issuer's company-level CEO, so regional roles
   * are not imported into the CEO feed.
   */
  if (
    requireCompanyCeo &&
    isRegionalExecutiveTitle(
      officerTitle
    )
  ) {
    return [];
...
export async function importForm4Trades(
  ...
  while (
    ...
    /*
     * Avoid sending a burst of consecutive upstream requests.
     * This small pause applies only between HermAI pages, not to
     * normal database reads performed by website visitors.
     */
...
groupedTrades =
  ...
  let refreshSkippedUnmatchedCount = 0;
  ...
  for (const trade of groupedTrades) {
    try {
      ...
      /*
       * Repair mode must never expand the dataset. It may refresh an
       * existing card or create a corrected compensation target only
       * when matching legacy cards already exist.
       */
      if (
        refreshExisting &&
        !existing
      ) {
        refreshSkippedUnmatchedCount += 1;
        continue;
      }
